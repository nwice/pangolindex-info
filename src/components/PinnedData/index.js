...
const RightColumn = styled.div`
  ...
  padding: 1.25rem;
  border-left: ${({ theme, open }) => '1px solid ' + theme.pinnedBorderColor};
  background-color: ${({ theme }) => theme.pinnedBackgroundColor};
  z-index: 9999;
  overflow: auto;
  :hover {
    cursor: pointer;
...
const SavedButton = styled(RowBetween)`
  ...
  border-bottom: ${({ theme, open }) => open && '1px solid' + theme.pinnedBorderColor};
  margin-bottom: ${({ open }) => open && ''};
...
function PinnedData({ history, open, setSavedOpen }) {
  ...
  const [useChartLock, setUseChartLock] = useState(true)

  const chartLockText = 'Turn off chart scrolling';
  return !open ? (
    <RightColumn open={open} onClick={() => setSavedOpen(true)}>
      ...
      <RightColumn gap="1rem" open={open}>
        ...
        <AutoColumn gap="40px" style={{ marginTop: '2rem' }}>
          ...
          <CheckBox checked={useChartLock} setChecked={() => setUseChartLock(!useChartLock)} text={'Chart Lock'} />
          <QuestionHelper text={chartLockText} />
        </AutoColumn>
